...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.vendite.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.vendites.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="data_inserimento">{{ trans('cruds.vendite.fields.data_inserimento') }}</label>
                <input class="form-control date {{ $errors->has('data_inserimento') ? 'is-invalid' : '' }}" type="text" name="data_inserimento" id="data_inserimento" value="{{ old('data_inserimento') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('ora_inserimento') ? 'is-invalid' : '' }}" type="text" name="ora_inserimento" id="ora_inserimento" value="{{ old('ora_inserimento') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('operatore') ? 'is-invalid' : '' }}" type="text" name="operatore" id="operatore" value="{{ old('operatore', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('servizio') ? 'is-invalid' : '' }}" name="servizio_id" id="servizio_id">
                    @foreach($servizios as $id => $servizio)
                        <option value="{{ $id }}" {{ old('servizio_id') == $id ? 'selected' : '' }}>{{ $servizio }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantita') ? 'is-invalid' : '' }}" type="number" name="quantita" id="quantita" value="{{ old('quantita', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_agente') ? 'is-invalid' : '' }}" name="id_agente_id" id="id_agente_id">
                    @foreach($id_agentes as $id => $id_agente)
                        <option value="{{ $id }}" {{ old('id_agente_id') == $id ? 'selected' : '' }}>{{ $id_agente }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_point') ? 'is-invalid' : '' }}" name="id_point_id" id="id_point_id">
                    @foreach($id_points as $id => $id_point)
                        <option value="{{ $id }}" {{ old('id_point_id') == $id ? 'selected' : '' }}>{{ $id_point }}</option>
                    @endforeach
                </select>
