...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.inserimentoSoglieRange.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.inserimento-soglie-ranges.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('gara') ? 'is-invalid' : '' }}" name="gara_id" id="gara_id">
                    @foreach($garas as $id => $gara)
                        <option value="{{ $id }}" {{ old('gara_id') == $id ? 'selected' : '' }}>{{ $gara }}</option>
                    @endforeach
                </select>
                @if($errors->has('gara'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('soglia_minima') ? 'is-invalid' : '' }}" type="number" name="soglia_minima" id="soglia_minima" value="{{ old('soglia_minima', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('soglia_massima') ? 'is-invalid' : '' }}" type="number" name="soglia_massima" id="soglia_massima" value="{{ old('soglia_massima', '') }}" step="0.01">
