...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.soggettiRelationship.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.soggetti-relationships.update", [$soggettiRelationship->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('brand') ? 'is-invalid' : '' }}" name="brand_id" id="brand_id">
                    @foreach($brands as $id => $brand)
                        <option value="{{ $id }}" {{ (old('brand_id') ? old('brand_id') : $soggettiRelationship->brand->id ?? '') == $id ? 'selected' : '' }}>{{ $brand }}</option>
                    @endforeach
                </select>
                @if($errors->has('brand'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('inizio') ? 'is-invalid' : '' }}" type="text" name="inizio" id="inizio" value="{{ old('inizio', $soggettiRelationship->inizio) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fine') ? 'is-invalid' : '' }}" type="text" name="fine" id="fine" value="{{ old('fine', $soggettiRelationship->fine) }}">
                @if($errors->has('fine'))
                    <div class="invalid-feedback">
                        {{ $errors->first('fine') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\SoggettiRelationship::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $soggettiRelationship->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_dealer') ? 'is-invalid' : '' }}" name="id_dealer_id" id="id_dealer_id">
                    @foreach($id_dealers as $id => $id_dealer)
                        <option value="{{ $id }}" {{ (old('id_dealer_id') ? old('id_dealer_id') : $soggettiRelationship->id_dealer->id ?? '') == $id ? 'selected' : '' }}>{{ $id_dealer }}</option>
                    @endforeach
                </select>
