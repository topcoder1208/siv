...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.notizie.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.notizies.update", [$notizie->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="titolo">{{ trans('cruds.notizie.fields.titolo') }}</label>
                <input class="form-control {{ $errors->has('titolo') ? 'is-invalid' : '' }}" type="text" name="titolo" id="titolo" value="{{ old('titolo', $notizie->titolo) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('brand') ? 'is-invalid' : '' }}" name="brand_id" id="brand_id" required>
                    @foreach($brands as $id => $brand)
                        <option value="{{ $id }}" {{ (old('brand_id') ? old('brand_id') : $notizie->brand->id ?? '') == $id ? 'selected' : '' }}>{{ $brand }}</option>
                    @endforeach
                </select>
                @if($errors->has('brand'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('descrizione_breve') ? 'is-invalid' : '' }}" name="descrizione_breve" id="descrizione_breve">{!! old('descrizione_breve', $notizie->descrizione_breve) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('inizio_visualizzazione') ? 'is-invalid' : '' }}" type="text" name="inizio_visualizzazione" id="inizio_visualizzazione" value="{{ old('inizio_visualizzazione', $notizie->inizio_visualizzazione) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fine_visualizzazione') ? 'is-invalid' : '' }}" type="text" name="fine_visualizzazione" id="fine_visualizzazione" value="{{ old('fine_visualizzazione', $notizie->fine_visualizzazione) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('visualizza_primapagina') ? 'is-invalid' : '' }}" type="text" name="visualizza_primapagina" id="visualizza_primapagina" value="{{ old('visualizza_primapagina', $notizie->visualizza_primapagina) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" type="text" name="link" id="link" value="{{ old('link', $notizie->link) }}">
                @if($errors->has('link'))
                    <div class="invalid-feedback">
                        {{ $errors->first('link') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('autorizzati') ? 'is-invalid' : '' }}" name="autorizzati" id="autorizzati">
                    ...
                    @foreach(App\Models\Notizie::AUTORIZZATI_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('autorizzati', $notizie->autorizzati) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
