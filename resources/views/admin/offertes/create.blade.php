...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.offerte.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.offertes.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('inizio_validita') ? 'is-invalid' : '' }}" type="text" name="inizio_validita" id="inizio_validita" value="{{ old('inizio_validita') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fine_validita') ? 'is-invalid' : '' }}" type="text" name="fine_validita" id="fine_validita" value="{{ old('fine_validita') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tecnologia') ? 'is-invalid' : '' }}" name="tecnologia_id" id="tecnologia_id" required>
                    @foreach($tecnologias as $id => $tecnologia)
                        <option value="{{ $id }}" {{ old('tecnologia_id') == $id ? 'selected' : '' }}>{{ $tecnologia }}</option>
                    @endforeach
                </select>
