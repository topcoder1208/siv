    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.provinces.update", [$province->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('provincia') ? 'is-invalid' : '' }}" type="text" name="provincia" id="provincia" value="{{ old('provincia', $province->provincia) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('regione') ? 'is-invalid' : '' }}" name="regione_id" id="regione_id">
                    @foreach($regiones as $id => $regione)
                        <option value="{{ $id }}" {{ (old('regione_id') ? old('regione_id') : $province->regione->id ?? '') == $id ? 'selected' : '' }}>{{ $regione }}</option>
                    @endforeach
                </select>
