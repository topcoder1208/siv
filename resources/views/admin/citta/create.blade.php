...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.cittum.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.citta.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="cap">{{ trans('cruds.cittum.fields.cap') }}</label>
                <input class="form-control {{ $errors->has('cap') ? 'is-invalid' : '' }}" type="text" name="cap" id="cap" value="{{ old('cap', '') }}" required>
                @if($errors->has('cap'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('citta') ? 'is-invalid' : '' }}" type="text" name="citta" id="citta" value="{{ old('citta', '') }}" required>
                @if($errors->has('citta'))
                    <div class="invalid-feedback">
                        {{ $errors->first('citta') }}
                    </div>
